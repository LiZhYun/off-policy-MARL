...
from offpolicy.algorithms.macpf.algorithm.actor_critic import MACPF_Actor, MACPF_Critic
from offpolicy.algorithms.macpf.algorithm.mq_mixer import M_QMixer
from offpolicy.utils.util import is_discrete, is_multidiscrete, get_dim_from_space, DecayThenFlatSchedule, soft_update, hard_update, \
    gumbel_softmax, onehot_from_logits, gaussian_noise, avail_choose, to_numpy, FixedCategorical, FixedNormal, to_torch


class MACPFPolicy(MLPPolicy):
    """
    ...
    def __init__(self, config, policy_config, target_noise=None, td3=False, train=True):
        ...
        actor_class = MACPF_Actor
        critic_class = MACPF_Critic

        self.actor = actor_class(self.args, self.obs_dim, self.act_dim, self.device)
        self.critic = critic_class(self.args, self.obs_dim, self.act_dim, self.device)

        self.target_actor = actor_class(self.args, self.obs_dim, self.act_dim, self.device)
        self.target_critic = critic_class(self.args, self.obs_dim, self.act_dim, self.device)

        # sync the target weights
        self.target_actor.load_state_dict(self.actor.state_dict())
        self.target_critic.load_state_dict(self.critic.state_dict())

        self.mixer = M_QMixer(self.args, self.args.num_agents, self.central_obs_dim, self.device)

        self.target_mixer = M_QMixer(self.args, self.args.num_agents, self.central_obs_dim, self.device)
        self.target_mixer.load_state_dict(self.mixer.state_dict())

        if not self.discrete:
            self.std_x_coef = 1.
            self.std_y_coef = 0.5
            log_std = torch.ones(self.act_dim) * self.std_x_coef
            self.log_std = torch.nn.Parameter(log_std)

        if train:
            self.critic_parameters = []
            self.critic_parameters += self.critic.parameters()
            self.critic_parameters += self.mixer.parameters()
            self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=self.lr, eps=self.opti_eps, weight_decay=self.weight_decay)
            self.critic_optimizer = torch.optim.Adam(self.critic_parameters, lr=self.lr, eps=self.opti_eps, weight_decay=self.weight_decay)
    ...
    def get_actions(self, obs, available_actions=None, t_env=None, explore=False, use_target=False, use_gumbel=False, parents_actions=None, execution_mask=None, dep_mode=False):
        """See parent class."""
        ...
        if use_target:
            ...
            actor_out = self.actor(obs, parents_actions, execution_mask, dep_mode)
        ...
        if self.discrete:
            if self.multidiscrete:
                ...
                dist = FixedCategorical(logits=actor_out)
                action_log_prob = dist.log_probs(torch.argmax(to_torch(actions),-1).to(self.device))
        else:
            action_std = torch.sigmoid(self.log_std / self.std_x_coef) * self.std_y_coef
            dist = FixedNormal(actor_out, action_std)
            if explore:
                actions = dist.sample()
            ...
            #     actions = gaussian_noise(actor_out.shape, self.target_noise) + actor_out
                ...
                actions = actor_out
            action_log_prob = dist.log_probs(actions)
        ...
        return actions, eps, action_log_prob
    ...
    def soft_target_updates(self):
        ...
        soft_update(self.target_mixer, self.mixer, self.args.tau)
