...
import copy
import itertools
from offpolicy.utils.util import huber_loss, mse_loss, to_torch, generate_mask_from_order
...
class MACPF(Trainer):
    ...
    def get_update_info(self, update_policy_id, obs_batch, act_batch, nobs_batch, navail_act_batch, dep_mode=False):
        ...
        for p_id in self.policy_ids:
            ...
            with torch.no_grad():
                ...
                for agent_idx in range(self.num_agents):
                    ...
                    actions[:,agent_idx] = pol_nact.cpu().detach().numpy()
                    naction_log_probs[:,agent_idx] = naction_log_prob.cpu().detach().numpy()
                    # ind_agent_nacts = pol_nact.cpu().split(split_size=batch_size, dim=0)
            # cat to form the centralized next step actions
            cent_nact.append(actions)
            cent_naction_log_probs.append(naction_log_probs)
        ...
        return cent_act, replace_ind_start, cent_nact, cent_naction_log_probs
    ...
    def shared_train_policy_on_batch(self, update_policy_id, batch, dep_mode=False):
        ...
        cent_act, replace_ind_start, nact_batch, naction_log_probs = self.get_update_info(update_policy_id, obs_batch, act_batch, nobs_batch, navail_act_batch, dep_mode)
        ...
        act_batch = to_torch(act_batch[update_policy_id]).to(**self.tpdv)
        nact_batch = to_torch(nact_batch[0]).to(**self.tpdv).permute(1,0,2)
        
        agent_order = torch.stack([to_torch(np.arange(self.num_agents)).to(**self.tpdv) for _ in range(batch_size)])
        execution_masks_batch = generate_mask_from_order(
            agent_order, ego_exclusive=False).to(**self.tpdv).permute(1,0,2)  # [bs, n_agents, n_agents]
        # critic update
        with torch.no_grad():
            nparents_actions = nact_batch.unsqueeze(2).repeat(1,1,self.num_agents,1).reshape(batch_size*self.num_agents,self.num_agents,-1)           
            next_step_Qs = update_policy.target_critic(nobs_batch[update_policy_id].reshape(batch_size*self.num_agents,-1), nact_batch.reshape(batch_size*self.num_agents,-1), nparents_actions, execution_masks_batch.reshape(batch_size*self.num_agents,-1), dep_mode)

        next_step_Q = next_step_Qs
        ...
        next_step_Q_tot = update_policy.target_mixer(next_step_Q, cent_nobs).squeeze(-1).to(**self.tpdv)

        rewards = to_torch(rewards).to(**self.tpdv).view(-1, 1)
        dones_env = to_torch(dones_env).to(**self.tpdv).view(-1, 1)

        naction_log_probs = to_torch(naction_log_probs[0]).to(**self.tpdv).sum((1,2)).unsqueeze(-1)
        if self.use_popart:
            target_Qs = rewards + self.args.gamma * (1 - dones_env) * (self.value_normalizer[p_id].denormalize(next_step_Q_tot)-self.alpha*naction_log_probs)
            target_Qs = self.value_normalizer[p_id](target_Qs)
        else:
            target_Qs = rewards + self.args.gamma * (1 - dones_env) * (next_step_Q_tot-self.alpha*naction_log_probs)

        parents_actions = act_batch.unsqueeze(2).repeat(1,1,self.num_agents,1).reshape(batch_size*self.num_agents,self.num_agents,-1)          
        predicted_Qs = update_policy.critic(obs_batch[update_policy_id].reshape(batch_size*self.num_agents,-1), act_batch.reshape(batch_size*self.num_agents,-1), parents_actions, execution_masks_batch.reshape(batch_size*self.num_agents,-1), dep_mode)

        curr_Q_tot = update_policy.mixer(predicted_Qs, cent_obs).squeeze(-1).to(**self.tpdv)
        ...
        critic_grad_norm = torch.nn.utils.clip_grad_norm_(update_policy.critic_parameters,
        ...
        if update_actor:
            ...
            parents_actions = act_batch.unsqueeze(2).repeat(1,1,self.num_agents,1).reshape(batch_size*self.num_agents,self.num_agents,-1)       
            pol_acts, _, action_log_probs = update_policy.get_actions(pol_agents_obs_batch, pol_agents_avail_act_batch, parents_actions=parents_actions, execution_mask=execution_masks_batch.reshape(batch_size*self.num_agents,-1), dep_mode=dep_mode)
            # separate into individual agent batches
            actor_loss = (self.alpha * action_log_probs - predicted_Qs.detach()).mean()
