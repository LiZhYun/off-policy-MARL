...
from offpolicy.runner.mlp.base_runner_macpf import MlpRunner
    ...
    def collect_rollout(self, explore=True, training_episode=True, warmup=False):
        ...
        for step in range(self.episode_length):
            ...
            if warmup:
                ...
                actions = np.zeros((n_rollout_threads, self.num_agents, self.policy_act_dim['policy_0']))
                for agent_idx in range(self.num_agents):
                    tmp_execution_mask = torch.stack([torch.ones(n_rollout_threads)] * agent_idx +
                                                [torch.zeros(n_rollout_threads)] *
                                                (self.num_agents - agent_idx), -1)
                    action, _, _ = policy.get_actions(obs[:,agent_idx],
                                                        avail_acts[:,agent_idx],
