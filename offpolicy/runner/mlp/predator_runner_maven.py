...
from offpolicy.runner.mlp.base_runner_maven import MlpRunner

class PreyRunner(MlpRunner):
    ...
    def eval(self):
        ...
        # eval_infos['eval_average_episode_rewards'] = eval_infos['average_episode_rewards']
    ...
    def collect_rollout(self, explore=True, training_episode=True, warmup=False):
        ...
        self.noise = self.noise_distrib.sample((n_rollout_threads,)).unsqueeze(-2).repeat(1, self.num_agents, 1).detach().cpu().numpy()

        for step in range(self.episode_length):
            obs_batch = np.concatenate(obs)
            noise_batch = np.concatenate(self.noise)
