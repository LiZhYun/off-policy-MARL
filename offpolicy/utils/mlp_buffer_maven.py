...
class MlpReplayBuffer(object):
    def __init__(self, args, policy_info, policy_agents, buffer_size, use_same_share_obs, use_avail_acts,
        ...
        self.policy_buffers = {p_id: MlpPolicyBuffer(args, buffer_size,
    ...
    def insert(self, num_insert_steps, obs, share_obs, acts, rewards,
               ...
               avail_acts, next_avail_acts, noise):
        """
    ...
    def sample(self, batch_size):
        ...
        obs, share_obs, acts, rewards, next_obs, next_share_obs, dones, dones_env, valid_transition, avail_acts, next_avail_acts, noises = {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}
        for p_id in self.policy_info.keys():
            obs[p_id], share_obs[p_id], acts[p_id], rewards[p_id], next_obs[p_id], next_share_obs[p_id], \
            dones[p_id], dones_env[p_id], valid_transition[p_id], avail_acts[p_id], next_avail_acts[p_id], noises[p_id] = \
            self.policy_buffers[p_id].sample_inds(inds)

        return obs, share_obs, acts, rewards, next_obs, next_share_obs, dones, dones_env, valid_transition, avail_acts, next_avail_acts, None, None, noises


class MlpPolicyBuffer(object):

    def __init__(self, args, buffer_size, num_agents, obs_space, share_obs_space, act_space, use_same_share_obs,
        ...
        self.noise = np.zeros(
            (self.buffer_size, self.num_agents, self.noise_dim), dtype=np.float32)
    ...
    def insert(self, num_insert_steps, obs, share_obs, acts, rewards,
               next_obs, next_share_obs, dones, dones_env, valid_transition,
               avail_acts=None, next_avail_acts=None, noise=None):
        """
        ...
        self.noise[idx_range] = noise.copy()
    ...
    def sample_inds(self, sample_inds):
        ...
        noise = _cast(self.noise[sample_inds])
        ...
        return obs, share_obs, acts, rewards, next_obs, next_share_obs, dones, dones_env, valid_transition, avail_acts, next_avail_acts, noise
